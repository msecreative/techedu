        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{route('task.update', $task->id)}}" method="POST" enctype="multipart/form-data">@csrf @method('PUT')
                        <div class="mt-6 flex justify-between gap-4">
                            <div class="mt-6 flex-1">
                                <label for="name" class="techedu-label">Name</label>
                                <input type="text" name="name" id="name" value="{{$task->name}}" class="techedu-input">
                        ...
                        <div class="flex justify-between gap-4">
                            <div class="mt-6 flex-1">
                                ...
                                <input type="number" name="price" value="{{$task->price}}" id="price" class="techedu-input">
                                @error('price')
                                    <p class="text-red-600">{{$message}}</p>
                                @enderror
                            </div>
                        ...
                        <div class="flex justify-between gap-4">
                            <div class="mt-6 flex-1">
                                ...
                                <textarea type="text" rows="8" name="description" id="description" class="techedu-input">{{$task->description}}</textarea>
                                @error('description')
                                    <p class="text-red-600">{{$message}}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mt-6">
                            <button class="rounded bg-sky-400 uppercase text-white px-6 py-2">Update</button>
                        </div>
                    </form>
                </div>
            </div>
